...
class PygAnimation(object):
    def __init__(self, frames, loop=True):
        ...
        if frames != '_copy':  # ('_copy' is passed for frames by the getCopies() method)
            ...
            for i in range(self.numFrames):
                ...
                assert type(frame[0]) in (str, pygame.Surface), 'Frame %s image must be a string filename' \
                                                                ' or a pygame.Surface' % i
    ...
    def anchor(self, anchorPoint=NORTHWEST):
        ...
        for i in range(len(self._images)):
            ...
            # NOTE: This makes changes to the original images in self._images, not the transformed
            # images in self._transformedImages
